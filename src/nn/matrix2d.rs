...
use std::ops::{Sub,AddAssign,Mul,SubAssign,DivAssign,MulAssign};
...
impl Matrix2D{
    ...
    pub fn min(&self)->f32{
        let a:Vec<&f32> = self.matrix.iter().map(|row| row.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap()).collect();
    ...
    //一维数组转换二维矩阵
    pub fn from(values:&Vec<f32>)->Matrix2D{
        Matrix2D{ matrix:values.clone().into_iter().map(|x| vec![x]).collect() }
    }
