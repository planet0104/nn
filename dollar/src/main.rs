...
//
const NumUnistrokes:usize = 16;
const NumPoints:usize = 64;
const SquareSize:f64 = 250.0;
const Origin:Point = Point{x: 0.0, y:0.0};
//const Diagonal:f64 = (SquareSize * SquareSize + SquareSize * SquareSize).sqrt();
const Diagonal:f64 = 353.5533905932738;
const HalfDiagonal:f64 = 0.5 * Diagonal;
//const AngleRange:f64 = deg2rad(45.0);
const AngleRange:f64 = 0.7853981633974483;
//const AnglePrecision:f64 = deg2rad(2.0);
const AnglePrecision:f64 = 0.03490658503988659;
...
impl Point{
    fn new<T: Into<f64>>(x: T, y: T) -> Point{
...
impl Unistroke{
    fn new(name: &str, points: Vec<Point>) -> Unistroke{
        let points = resample(points, NumPoints);
        let radians = indicative_angle(&points);
        let points = rotate_by(&points, -radians);
        let points = scale_to(&points, SquareSize);
...
impl Result{
    fn new(name:&str, score:f64, ms: f64) -> Result{ // constructor
        Result{ name: name.to_string(), score, ms}
    }
}
...
impl DollarRecognizer{
    ...
    pub fn delete_user_gestures(&mut self) -> usize{
        self.unistrokes.resize(NumUnistrokes, Unistroke::default());
        NumUnistrokes
    }
    ...
    pub fn recognize(&self, points:Vec<Point>, use_protractor: bool) -> Result{
        ...
        let mut u = -1;
        for i in 0..self.unistrokes.len(){ // for each unistroke
            let mut d = 0.0;
            if use_protractor{ // for Protractor
                d = optimal_cosine_distance(&self.unistrokes[i].vector, &vector);
            }else{ // Golden Section Search (original $1)
                d = distance_at_best_angle(&points, &self.unistrokes[i], -AngleRange, AngleRange, AnglePrecision);
            }
            if d < b {
        ...
        if u == -1{
            ...
            Result::new(&self.unistrokes[u as usize].name, if use_protractor{ 1.0 / b }else{ 1.0 - b / HalfDiagonal}, t1)
        }
    }
}
...
fn optimal_cosine_distance(v1:&Vec<f64>, v2:&Vec<f64>) -> f64{ // for Protractor
	let mut a = 0.0;
	let mut b = 0.0;
...
fn distance_at_best_angle(points:&Vec<Point>, t:&Unistroke, mut a:f64, mut b:f64, threshold:f64) -> f64{
...
fn path_distance(pts1: &Vec<Point>, pts2: &Vec<Point>) -> f64{
	let mut d = 0.0;
	for i in 0..pts1.len(){ // assumes pts1.length == pts2.length
		d += distance(&pts1[i], &pts2[i]);
    }
	d / pts1.len() as f64
}

fn vectorize(points: &Vec<Point>) -> Vec<f64>{ // for Protractor
	let mut sum = 0.0;
	let mut vector = vec![];
	for point in points {
		vector.push(point.x);
...
fn translate_to(points: &Vec<Point>, pt: &Point) -> Vec<Point>{ // translates points' centroid
...
fn rotate_by(points:&Vec<Point>, radians: f64) -> Vec<Point>{ // rotates points around centroid
	let c = centroid(points);
	let cos = radians.cos();
	let sin = radians.sin();
	let mut newpoints = vec![];
...
fn scale_to(points:&Vec<Point>, size: f64) -> Vec<Point>{ // non-uniform scale; assumes 2D gestures (i.e., no lines)
...
fn resample(mut points: Vec<Point>, n: usize) -> Vec<Point>{
	...
	if newpoints.len() == n as usize - 1{ // somtimes we fall a rounding-error short of adding the last point, so add it if so
		newpoints.push(Point::new(points[points.len() - 1].x, points[points.len() - 1].y));
    }
	newpoints
}
